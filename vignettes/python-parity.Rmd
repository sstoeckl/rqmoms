...
if (has_py) {
  ...
  try({
    ...
    reticulate::py_run_string("import numpy, qmoms")  # verify both
    ok <- TRUE
  }, silent = TRUE)
  has_py <- ok
}
...
if (!has_py) {
  cat("**Python parity is disabled.** Set `RQMOMS_BUILD_PY=1` and either\n",
      "`RETICULATE_PYTHON` to a Python with `qmoms`, or `RQMOMS_VENV` to a venv.\n")
}
