...
# run with Python only when explicitly enabled
...
if (has_py) {
  ...
  try({
    ...
    if (nzchar(rp)) {
      # CI: bind to toolcache python prepared by setup-python
      reticulate::use_python(rp, required = TRUE)
    } else {
      # Local: optional virtualenv
      venv <- Sys.getenv("RQMOMS_VENV", file.path("python", ".venv"))
      if (!dir.exists(venv)) stop("RQMOMS_VENV not found: ", venv)
      reticulate::use_virtualenv(venv, required = TRUE)
    }
    # sanity: both must import
    reticulate::py_run_string("import numpy, qmoms")
    ok <- TRUE
  }, silent = TRUE)
  if (!ok) {
    has_py <- FALSE
    why <- "Could not import numpy/qmoms in the configured Python."
  }
} else {
  if (!requireNamespace("reticulate", quietly = TRUE))
    why <- "R package 'reticulate' is not installed."
  else
    why <- "RQMOMS_BUILD_PY != 1."
}

if (!has_py) {
  cat('<div class="alert alert-warning" role="alert">',
      '<strong>Python parity is disabled.</strong> ',
      'Set <code>RQMOMS_BUILD_PY=1</code> and either ',
      '<code>RETICULATE_PYTHON</code> (toolcache/system Python with qmoms installed) ',
      'or <code>RQMOMS_VENV</code> (a virtualenv). ',
