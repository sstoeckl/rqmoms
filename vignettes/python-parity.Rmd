...
if (has_py) {
  ...
  try({
    ...
    # show which interpreter rmarkdown is actually using
    reticulate::py_run_string('import sys; print("Knit using:", sys.executable)')

    # Try imports separately to get a clear error
    try(reticulate::py_run_string('import numpy'), silent = TRUE)
    try(reticulate::py_run_string('import qmoms'), silent = TRUE)

    # If either failed, raise a combined message
    reticulate::py_run_string('import importlib; mod_err=None')
    ok <- TRUE
  }, silent = TRUE)
  if (!ok) {
    has_py <- FALSE
    why <- "Could not import numpy/qmoms in vignette session."
  }
} else {
  why <- "RQMOMS_BUILD_PY != 1 or 'reticulate' missing."
}
