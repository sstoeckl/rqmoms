...
if (has_py && requireNamespace("reticulate", quietly = TRUE)) {
  # Wichtig: keine geerbte Toolcache-Bindung
  Sys.unsetenv("RETICULATE_PYTHON")
  venv <- Sys.getenv("RQMOMS_VENV", file.path("python",".venv"))
  ok <- FALSE
  try({
    reticulate::use_virtualenv(venv, required = TRUE)
    reticulate::py_run_string(paste("import numpy as np; import qmoms; & ",
...
if (!has_py) {
  cat("**Python parity is disabled.** Set `RQMOMS_BUILD_PY=1` and point `RQMOMS_VENV` to your virtualenv to run the comparison.\n")
}
```
